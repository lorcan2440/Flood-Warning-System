...
class MonitoringStation:
    ...
    '''
    This class represents a water-level monitoring station. There are over 2000 such stations
    across England, which record the water level at fixed locations on various rivers,
    coastal sites and aquifers.
    ...
    A small number of the stations are in Scotland or Wales, close to the English border and on
    rivers which flow into England.

    It is not recommended to build this class manually - use the factory functions, i.e.
    ```
    from floodsystem.stationdata import build_station_list
    stations = build_station_list()
    ```
    which gives a list of fully built `MonitoringStation`s. To set all `latest_level`s, call
    ```
    ...
    `label` (str): the name of the station, usually named after the surrounding village
    or district. Not guaranteed to be unique.

    `coord` (tuple): a coordinate pair giving the (latitude, longitude) position in degrees
    of the station. Available to 6 decimal places, or a resolution of < 10 cm in England.

    `typical_range` (tuple): a pair of (smaller value, larger value) giving the interval
    within which the middle 90% of flood level data (in metres) since recording lies,
    or ≈1.64 standard deviations below and above the mean. Available to 3 decimal places,
    or a resolution of 1 mm. Can be reliably assumed to be constant over time, although
    ...
    `latest_recorded_datetime` (datetime.datetime, default = None): the date and time when the
    most recent water level was recorded at this station. Stations are scheduled to record at
    15-minute intervals starting on the hour, i.e. at hh:00:00, hh:15:00, hh:30:00, hh:45:00...
    ...
    `is_tidal` (bool, default = False): whether or not this station measures coastal water
    levels. If true, the water level tends to fluctuate periodically by day and night due
    to the natural effect of tides.

    `is_groundwater` (bool, default = False): whether or not this station measures
    groundwater levels.

    `station_id` (str, default = None): a unique URL string giving access to its data.

    `url_id` (str, default = None): the RLOI (River Levels On the Internet) ID, used to
    access the webpage associated with the station.

    `record_range` (tuple, default = None): a pair of (min, max) giving the lowest ever and
    highest ever water levels (in metres) since recording for the station began. This value
    is manually verified to prevent erroneous spikes from setting this. Recorded to 3 decimal
    places, or a resolution of 1 mm.

    `upstream_url_id` (str, default = None): the url_id of the nearest upstream station
    (closer to the source, against the flow) to this station,
    or None if there is nothing upstream.

    `downstream_url_id` (str, default = None): the url_id of the nearest downstream station
    (further from the source, with the flow) to this station,
    or None if there is nothing downstream.

    ### Returns

    A `MonitoringStation` instance, which can be passed (either individually or a list
    of such objects) into a variety of API functions available in the separate modules.
    '''

    def __init__(self, measure_id: str, label: str, coord: tuple[float],
            typical_range: tuple[float], **kwargs):
        ...
        # default values
        _default_kwargs = {
            'latest_level': None, 'latest_recorded_datetime': None, 'url': None,
            'town': None, 'river': None, 'is_tidal': False, 'is_groundwater': False,
            'station_id': None, 'url_id': None, 'record_range': None,
            'upstream_url_id': None, 'downstream_url_id': None, 'station_type': None}
    ...
    def typical_range_consistent(self) -> bool:
        ...
        try:
            ...
            if low_val is None or high_val is None or low_val > high_val \
                    or not isinstance(low_val, (float, int)) \
                    or not isinstance(high_val, (float, int)) \
                    or low_val == high_val:
    ...
    def relative_water_level(self) -> float:
        ...
        if self.typical_range_consistent() and isinstance(self.latest_level, (float, int)):
            diff = self.typical_range[1] - self.typical_range[0]
            return (self.latest_level - self.typical_range[0]) / diff
        else:
            return None
